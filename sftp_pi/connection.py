...
class Connection (dict):
    ...
    def upload_file(self, local_file_path, remote_path):
        self.connection.put(f'{local_file_path}', f'{remote_path}' ,confirm=False)
    ...
    def upload_dir(self, local_dir, remote_dir, dir_exclusions: [str], file_suffix_exclusions: [str]):
        for entry in os.listdir(local_dir):
            ...
            print(f'Uploading.\tlocal: {local_path}\t\tremote: {remote_path}')
            if not os.path.isfile(local_path):
                try:
                    if local_path.split('\\')[-1] in dir_exclusions:
                        continue
                    else:
                        self.connection.mkdir(remote_path)
                except OSError as e:
                    print(f'error making dir  {remote_path}:   {e}')
